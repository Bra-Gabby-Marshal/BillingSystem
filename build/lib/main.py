...
class Bill_App:
    def __init__(self, root):
        ...
        self.root.title("Billing Software|By: Bra Gabby Marshal")
        bg_color = "#074463"
        title = Label(self.root, text="Gabby May Co.Limited Billing Software", bd=12, relief=GROOVE, bg=bg_color, fg="white", font=("times new roman", 30, "bold"), pady=2).pack(fill= X)
        ...
        cphn_txt = Entry(F1, width=15, textvariable=self.c_phon, font="arial 15", bd=7, relief=SUNKEN).grid(row=0, column=3, pady=10)
        ...
        bill_btn = Button(F1, text="Search",command=self.find_bill, width=10, bd=7, font="arial 12 bold").grid(row=0, column=6, padx=19, pady=10)
        ...
        c1_lbl = Label(F4, text="Maze", font=("times new roman", 16, "bold"), bg=bg_color, fg="lightgreen").grid(row=0, column=0, padx=10, pady=10, sticky="w")
        ...
        c6_txt = Entry(F4, width=10, textvariable=self.spite, font=("times new roman", 16, "bold"), bd=5, relief=SUNKEN).grid(row=5, column=1, padx=10, pady=10)
    ...
    def total(self):
        ...
        self.d_1_p = self.limca.get() * 40
        self.d_s_p = self.spite.get() * 60
    ...
    def welcome_bill(self):
        ...
        self.txtarea.insert(END, "\t Welcome Gabby-May Co.Ltd\n")
    ...
    def bill_area(self):
        ...
        if self.limca.get() != 0:
            self.txtarea.insert(END, f"\n Limca \t\t{self.limca.get()}\t\t{self.d_1_p}")
        if self.spite.get() != 0:
            self.txtarea.insert(END, f"\n Sprite \t\t{self.spite.get()}\t\t{self.d_s_p}")
